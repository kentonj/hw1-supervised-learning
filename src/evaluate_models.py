...
N_EPOCHS = 100000000 # number of epochs for neural network training - early stopping call back stops training
...
def train_model(algo, training_data, n_folds=5, n_chunks=5):
    ...
    if algo.framework == 'sklearn':
        ...
        early_stopping_condition = EarlyStopping(monitor='val_categorical_accuracy', min_delta=0.01, patience=300, restore_best_weights=True)
...
def main():
    ...
    #prep the datasets 
    [train_dataset, test_dataset], label_encoder = prep_data({'train':train_df, 'test':test_df}, shuffle_data=True, balance_method='downsample')
